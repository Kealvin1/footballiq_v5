/**
 * FootballIQ v5 — /netlify/functions/xg
...
exports.handler = async (event) => {
  ...
  const { team, competition } = event.queryStringParameters || {};

  if (!team) {
    return { statusCode: 400, headers: CORS, body: JSON.stringify({ error: "team param required" }) };
  }
  ...
  if (!league) {
    return {
      ...
      body: JSON.stringify({ success: false, reason: `${competition} not in Understat (EPL/La Liga/Bundesliga/Serie A/Ligue 1 only)` }),
    };
  }
  ...
  if (!raw) {
    return {
      statusCode: 200, headers: CORS,
      body: JSON.stringify({ success: false, reason: `No xG data found for "${team}" — team name may differ on Understat` }),
    };
  }
  ...
  if (!data) {
    return {
      statusCode: 200, headers: CORS,
      body: JSON.stringify({ success: false, reason: `Insufficient match data for "${team}" (need at least 3 results)` }),
    };
  }

  return {
    statusCode: 200, headers: CORS,
    body: JSON.stringify({ success: true, team, slug, season: `${usedYear}-${usedYear + 1}`, data }),
  };
};
